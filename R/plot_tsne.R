#' Plots tSNE of log map results
#' Runs tSNE on the log map vectors and builds a low dimensional plot,
...
#' plot_tsne(vectors = lm_vectors, phylogenomic = 1, phylogenomic_name = NULL, 
...
#'
#' @export
plot_tsne <- function(vectors, phylogenomic = NULL, phylogenomic_name = NULL, 
  ...
  tsne_df <- data.frame(pc_x = df$pc_x,
                        pc_y = df$pc_y,
                        Name = df$Name,
                        index = 1:nrow(df)) 
  tsne_x_df <- dplyr::arrange(tsne_df, dplyr::desc(abs(pc_x)))
  tsne_y_df <- dplyr::arrange(tsne_df, dplyr::desc(abs(pc_y)))
  return(list(plot = full_plot, tsne_x_info = tsne_x_df, tsne_y_info = tsne_y_df))
}
