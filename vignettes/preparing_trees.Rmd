...
1. Run `GToTree` to go from genome fasta files to individual protein alignment files for each gene. 
...
We will now use a few functions in `groves` to refine our set of genomes and genes. 

```{r, message = FALSE}
library(groves)
```

We will start by getting a list of genomes and genes included in our individual alignments. 
